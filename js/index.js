...
function display(deadline) {
  ...
  if (state == "start" || state == "pause")
    var percentage =
      ((workTimeDef * 60 - remainingTime) / (workTimeDef * 60)) * 100;
  else if (count % 4 == 0 && (state == "break" || state == "pauseBreak"))
    var percentage =
      ((longBreakTimeDef * 60 - remainingTime) / (longBreakTimeDef * 60)) * 100;
  else if (state == "break" || state == "pauseBreak")
    var percentage =
      ((breakTimeDef * 60 - remainingTime) / (breakTimeDef * 60)) * 100;
  else percentage = 0;
  ...
  switch (state) {
    ...
    case "break":
      ...
      if (count % 4 == 0) str = "<h1>Long Break</h1><br><h2>" + time + "</h2>";
      else str = "<h1>Break</h1><br><h2>" + time + "</h2>";
      break;
    case "pauseBreak":
...
function initializeClock(deadline) {
  ...
  function updateClock() {
    ...
    if (state != "pause" && state != "pauseBreak") remainingTime--;
    // if paused, stop the intervals
    else clearInterval(timeInterval);

    // checks if the current session is over
    if (time <= 0) {
      // plays a bell sound
      var audio = new Audio("./audio/cyberpunk_incomingcall.mp3");
      audio.play();
      // stops the intervals
      ...
      if (state == "start") {
        // if we were on a work session,
        //increment completed pomodoro counter
        count++;
        // long break if counter is multiple of 4
        if (count % 4 == 0) remainingTime = longBreakTimeDef * 60;
        // if not, normal break
        else remainingTime = breakTimeDef * 60;

        // change state to break and start the break timer
        state = "break";
        startBreak();
      } else {
        // if we were on a break
...
$(document).ready(function () {
  ...
  $(".work-plus").click(function () {
    if (state == "" || state == "pause" || state == "pauseBreak") {
      workTimeDef++;
      if (state == "pause") remainingTime += 60;
      deadline = new Date(Date.parse(new Date()) + remainingTime * 1000);
      display(deadline);
    }
  });
  ...
  $(".work-minus").click(function () {
    if (
      remainingTime > 61 &&
      workTimeDef > 1 &&
      (state == "" || state == "pause" || state == "pauseBreak")
    ) {
      workTimeDef--;
      if (state == "pause") remainingTime -= 60;
      deadline = new Date(Date.parse(new Date()) + remainingTime * 1000);
  ...
  $(".break-plus").click(function () {
    if (state == "" || state == "pause" || state == "pauseBreak") {
      breakTimeDef++;
      if (state == "pauseBreak") remainingTime += 60;
      deadline = new Date(Date.parse(new Date()) + remainingTime * 1000);
      display(deadline);
    }
  });
  ...
  $(".long-break-plus").click(function () {
    if (state == "" || state == "pause" || state == "pauseBreak") {
      longBreakTimeDef++;
      if (state == "pauseBreak" && count % 4 == 0) remainingTime += 60;
      deadline = new Date(Date.parse(new Date()) + remainingTime * 1000);
      display(deadline);
    }
  });
